...
class Backend(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.CommandNotFound):
            ...
            return await ctx.send("This command is on a cooldown! Try again later")
        errorem = discord.Embed(
            description=error,
            color=0xff0000
        )
        errorem.set_author(name="Error")
        errorem.set_footer(text="The error has been reported to the developer")
        ...
        log_channel = self.bot.get_channel(691373759893864529)
        await log_channel.send(embed=errorembed)
