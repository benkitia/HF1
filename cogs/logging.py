...
class Logging(commands.Cog):
    ...
    async def on_message_delete(self, message):
        ...
        logchannel = discord.utils.get(
            ctx.guild.text_channels, id=logchannelid)
        content = ctx.message.content
        if not content:
            content = "[No Content]"
        log = discord.Embed(
            description=f"{ctx.message.author.mention}'s message was deleted from {ctx.message.channel.mention}:", color=0xff1919)
        log.add_field(
            name="Content",
            value=content,
            inline=False
        ...
        log.set_author(icon_url=ctx.message.author.avatar_url,
    ...
    async def on_member_join(self, member):
        ...
        logchannelid = int(logchannelid)
        logchannel = discord.utils.get(
            member.guild.text_channels, id=logchannelid)
        global_inf_count = await self.db.infractions.count_documents({"Target": member.id, "Status": "Active"})
        dif = (datetime.datetime.utcfromtimestamp(
            time.time()) - member.created_at)
        dif = str(dif)
        print(dif.split(','))
        dif = dif.split(',')[0]
        log = discord.Embed(
        ...
        log.timestamp = datetime.datetime.utcnow()
        log.set_author(icon_url=member.avatar_url,
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        if before.nick != after.nick:
            ...
            logchannelid = int(logchannelid)
            logchannel = discord.utils.get(
                before.guild.text_channels, id=logchannelid)
            if before.nick == None:
            ...
            log.set_author(icon_url=before.avatar_url,
